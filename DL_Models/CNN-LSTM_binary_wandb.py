...
default_config = {'learning_rate': 0.001, 'filters': 20, 'kernel_size':3, 'lstmhidden': 20,
                  'batch_size': 32, 'dropout_rate': 0.2, 'strides': 1}
wandb.init(project='your-own-project-name', config=default_config)  # 초기값을 적용하여 wandb 실행
config = wandb.config

# 3. 데이터 로드
cnnlstmdata_x = np.load('Deep_input_X_scaled_36.npy')  # scaled features (16235, 108, 36)
cnnlstmdata_y = np.load('Deep_input_Y_BINARY.npy')     # binary labels (16235, 1)

x = cnnlstmdata_x
y = cnnlstmdata_y

# 4. 전체데이터를 트레이닝셋, 테스트셋으로 분리
x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=0.2, stratify=y)  # 회귀와 다르게 stratify=y를 통해 클래스 간 데이터 비율을 동일하게 설정해줌
...
model.add(Conv1D(filters=config.filters, kernel_size=config.kernel_size, padding='valid', strides=config.strides,
                 input_shape=(108, n_features)))
model.add(BatchNormalization())
model.add(Activation('relu'))
model.add(MaxPool1D())
## LSTM layer
model.add(LSTM(units=config.lstmhidden, activation='tanh'))
...
# 7. 모델 피팅
history = model.fit(x_train, y_train, epochs=100, batch_size=config.batch_size, validation_data=(x_val, y_val),
