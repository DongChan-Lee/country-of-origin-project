...
wandb.init(project='test1', config=default_config)   # 초기값을 적용하여 wandb 실행
config = wandb.config

# 3. 데이터 로드
cnnlstmdata_x = np.load('Deep_input_X_scaled_36.npy') # scaled features (16235, 108, 36)
cnnlstmdata_y = np.load('Deep_input_Y_MSE_scaled.npy') # scaled continuous labels (16235, 1)
...
# 7. 모델 피팅
history = model.fit(x_train, y_train, epochs=100, batch_size=config.batch_size, validation_split=0.2,
